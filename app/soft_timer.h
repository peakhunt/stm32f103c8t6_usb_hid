...
struct _soft_timer_elem
{
  ...
  uint32_t          tick;       /** absolute timeout tick count                       */
...
extern void soft_timer_add(SoftTimer* timer, SoftTimerElem* elem, uint32_t expires);
...
get_soft_tick_from_milsec(SoftTimer* timer, uint32_t milsec)
{
  uint32_t      tick,
                mod;
